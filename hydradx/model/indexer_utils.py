...
def get_omnipool_data_by_asset(
    ...
    for item in data:
        if item['assetId'] not in data_by_asset:
            ...
            last_block_id = data_by_asset[item['assetId']][-1]['paraBlockHeight']
            next_block_id = item['paraBlockHeight']
...
def get_stableswap_pools(block_number, stableswap_ids: list or str or int = None) -> dict[str: StableSwapPoolState]:
    ...
    stableswap_asset_query = f"""
        query assetInfoByAssetIds($stableswap_ids: [String!]) {{
            stableswapAssets(filter: {{poolId: {{in: $stableswap_ids}}}}) {{
                nodes {{
                    pool {{
                        id
                    }}
    ...
    """
    stableswap_query_data = query_indexer(
        URL_UNIFIED_PROD, stableswap_asset_query, variables={"stableswap_ids": stableswap_ids}
    )
    stableswap_asset_data = {}
    for node in sorted(
        stableswap_query_data['data']['stableswapAssets']['nodes'],
    ...
    while len(asset_ids_remaining) > 0 and queries < max_queries:
        ...
        for node in data['data']['stableswapAssetData']['nodes']:
            ...
            if pool_id not in stableswap_ids:
                continue
...
def get_omnipool_liquidity(
    ...
    asset_ids_remaining = [asset_id for asset_id in asset_ids_remaining if is_decimal(asset_id)]
    liquidity = {}
    lrna = {}
    shares = {}
    protocol_shares = {}
...
def get_omnipool_trades(
    ...
    for trade in data_all:
        ...
        trade.pop("args")
        trade.update(args)
        if 'assetIn' not in args:
            continue
        if 'assetOut' not in args:
            continue
...
def get_current_omnipool_router(block_number: int = None):
    if block_number is None:
        block_number = get_current_block_height()
...
def download_omnipool_spot_prices(tkn_id: str, denom_id: str, min_block: int, max_block: int, path: str):
    ...
    for i in range(max_block - min_block + 1):
        ...
        assert data_tkn['paraBlockHeight'] == data_denom['paraBlockHeight'], "Block number must match"
        block_no = int(data_tkn['paraBlockHeight'])
        assert block_no == min_block + i, "Block number must be sequential"
        hub_price_of_tkn = int(data_tkn['assetState']['d'][1]) / int(data_tkn['balances']['d'][0])
        hub_price_of_denom = int(data_denom['assetState']['d'][1]) / int(data_denom['balances']['d'][0])
